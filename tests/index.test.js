...
//     response = await request(app).get("/balances/inr");
//     expect(response.status).toBe(200);
//     expect(response.body["user1"]).toEqual({ balance: 50000, locked: 0 });
//     expect(response.body["user2"]).toEqual({ balance: 30000, locked: 0 });
...
//     expect(response.status).toBe(200);
//     expect(response.body.message).toBe(
//       "Minted 200 'yes' and 'no' tokens for user user1, remaining balance is 20000"
//     );
...
//       140: { total: 100, orders: { user1: 100 } },
//       150: { total: 100, orders: { user1: 100 } },
//     });
...
//     // Step 7: User2 places a buy order for 100 tokens, should match the lower price first (140)
//     response = await request(app).post("/order/buy").send({
//       userId: "user2",
//       stockSymbol: "ETH_USD_15_Oct_2024_12_00",
//       quantity: 100,
...
//     });
//     expect(response.status).toBe(200);
//     expect(response.body.message).toBe("Buy order matched at best price 140");
...
//     response = await request(app).get("/balances/inr");
//     expect(response.status).toBe(200);
//     expect(response.body["user2"]).toEqual({ balance: 16000, locked: 0 });
...
//     // Step 9: User2 places a buy order for 50 tokens, should partially match the 150 sell
//     response = await request(app).post("/order/buy").send({
//       userId: "user2",
//       stockSymbol: "ETH_USD_15_Oct_2024_12_00",
//       quantity: 50,
